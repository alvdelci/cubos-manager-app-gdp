...
const CustomerRegister = ({ show, handleClose }) => {
  ...
  const onSubmit = async (data) => {
    try {
      const newUser = await postCustomer(data);
      console.log(newUser);
    } catch (error) {
      console.error(error);
    }
  ...
  const handleCep = async (param) => {
    if (param.length == 8) {
      ...
      setValue("cep", parseInt(param, 10));
    }
  };
